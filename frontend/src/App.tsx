...
function App() {
  ...
  // Fetch orders when staff dashboard is loaded
  useEffect(() => {
    if (user && user.role === 'staff' && currentPage === 'staff-dashboard') {
      fetchOrders();
    }
  }, [user, currentPage]);
  ...
  const fetchOrders = async () => {
    try {
      ...
      // Fallback to sample orders if API fails
      const sampleOrders = [
        {
          id: '1',
          items: [
          ...
          customerName: 'Malith Jayawardana',
          status: 'pending',
  ...
  const handleCompleteOrder = async () => {
    ...
    if (cart.length > 0) {
      try {
        ...
        // Add to local orders list if staff is viewing
        if (user?.role === 'staff') {
  ...
  const updateOrderStatus = async (orderId: string, status: 'pending' | 'in-progress' | 'completed') => {
    try {
